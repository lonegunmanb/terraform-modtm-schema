...
import (
	...
	"github.com/lonegunmanb/terraform-modtm-schema/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh Modtm schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-modtm-schema", options)
		if err != nil {
			return false, err
		}
