...
resource "modtm_telemetry" "test" {
  tags = {
    ...
    avm_git_org              = "Azure"
    avm_git_repo             = "terraform-provider-modtm"
  }
  nonce = 12345
  lifecycle {
...
const placeHolderTemplate = `package %s
...
func RefreshSchema(folder string) (version *version.Version, err error) {
	s, err := ExtractModTmProviderSchema()
	if err != nil {
		return nil, err
	}
...
func ExtractModTmProviderSchema() (*ProviderSchema, error) {
	...
	r := schema.Schemas["registry.terraform.io/azure/modtm"]

	_, versions, err := tf.Version(context.Background(), true)
	if err != nil {
		return nil, fmt.Errorf("error running version: %w", err)
	}
	v, ok := versions["registry.terraform.io/azure/modtm"]
	if !ok {
		return nil, fmt.Errorf("error getting modtm version")
	}
...
func saveRegisterCode(folder string, s *tfjson.ProviderSchema) error {
	...
	testTemplate := registerTestTemplate
	if len(s.ResourceSchemas) == 0 {
		testTemplate = strings.ReplaceAll(testTemplate, "assert.NotEmpty(t, generated.Resources)", "")
	}
	if len(s.DataSourceSchemas) == 0 {
		testTemplate = strings.ReplaceAll(testTemplate, "assert.NotEmpty(t, generated.DataSources)", "")
	}
	err = saveRegister(testTemplate, parameter, filepath.Join(folder, "register_test.go"))
	if err != nil {
...
func buildRegisterParameter(s *tfjson.ProviderSchema) RegisterParameter {
	parameter := RegisterParameter{
		...
		GoModule:          "terraform-modtm-schema",
	}
...
func saveDataSourceSchemas(folder string, s *tfjson.ProviderSchema) error {
	...
	err := save(filepath.Join(folder, string("data"), "placeholder.go"), []byte(fmt.Sprintf(placeHolderTemplate, "data")))
	if err != nil {
		return err
	}
	return nil
...
func saveResourceSchemas(folder string, s *tfjson.ProviderSchema) error {
	...
	err := save(filepath.Join(folder, string("resource"), "placeholder.go"), []byte(fmt.Sprintf(placeHolderTemplate, "resource")))
	if err != nil {
		return err
	}
	return nil
